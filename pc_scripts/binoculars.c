...
int Function_48(int iParam0) //Position: 0x29F3 / 10739
{
	...
	switch (iLocal_2)
	{
		...
		case 0x00000001:
			if (Function_70(&iParam0, &uVar0))
			{
				Function_61(&iParam0, &uVar0, &iLocal_3);
				iLocal_2 = 2;
			}
			iVar1 = 0;
			break;
		...
		case 0x00000003:
			if (!Function_52(&iParam0, &uVar0, &iLocal_3))
			{
				iLocal_2 = 4;
			}
			iVar1 = 0;
...
void Function_49(int iParam0, var uParam1, int iParam2) //Position: 0x2AAA / 10922
{
	...
	uVar0 = Function_60(&iParam0);
	iParam2 = 0;
	if (IS_GRINGO_COMPONENT_VALID(&uVar0))
	{
		GRINGO_UNLOAD_ANIMATION_FOR_USER(&uVar0, GET_OBJECT_FROM_ACTOR(&uParam1));
	}
	SET_PLAYER_CONTROL(0, 1, 0, 0);
	ENABLE_USE_CONTEXTS(1);
	fLocal_15 = (GET_CURRENT_GAME_TIME() - fLocal_15);
	Function_50(471, CEIL(fLocal_15), 0, 0);
	if ((((UI_ISACTIVE("BinocularsUI") || UI_ISACTIVE("Movies")) || IS_GAME_RESETTING()) || IS_OBJECT_VALID(&uLocal_7)) || IS_OBJECT_VALID(&uLocal_5))
	{
		UI_POP("BinocularsUI");
	}
	SET_USING_BINOCULARS(0);
	UNK_0xE6C1DBD9(0);
	if (IS_OBJECT_VALID(&uLocal_5))
	{
		DESTROY_CAMERA(&uLocal_5);
	}
	if (IS_OBJECT_VALID(&uLocal_7))
	{
		DESTROY_OBJECT(&uLocal_7);
	}
	ALLOW_GAME_CAMERA_AUTO_TILTING(iLocal_9, 2);
	ALLOW_GAME_CAMERA_AUTO_CENTERING(iLocal_10, 2);
	ALLOW_GAME_CAMERA_AUTO_TILTING(iLocal_11, 0);
	ALLOW_GAME_CAMERA_AUTO_CENTERING(iLocal_12, 0);
	ALLOW_GAME_CAMERA_AUTO_TILTING(iLocal_13, 1);
	ALLOW_GAME_CAMERA_AUTO_CENTERING(iLocal_14, 1);
	Global_99146 = 0;
...
int Function_50(int iParam0, bool bParam1, var uParam2, var uParam3) //Position: 0x2B94 / 11156
{
	int iVar0;
	int iVar1;
...
void Function_51(int iParam0) //Position: 0x2DBC / 11708
{
	switch (iParam0)
	{
		case 0x00000009:
...
bool Function_52(int iParam0, int iParam1, int iParam2) //Position: 0x2E5A / 11866
{
	var uVar0;
	float fVar1;
	var uVar2;
	...
	uVar0 = Function_60(&iParam0);
	if (iParam2 && !((Global_6621 || HUD_IS_FADING()) || HUD_IS_FADED()))
	{
		if (IS_GRINGO_COMPONENT_VALID(&uVar0) && iLocal_4)
		{
			if (DECOR_CHECK_EXIST(&iParam1, "BinocularsStart"))
			{
				if (Function_54())
				{
					DECOR_REMOVE(&iParam1, "BinocularsStart");
				}
			}
			if (DECOR_CHECK_EXIST(&Global_54076, "nquitbinos"))
			{
				UI_POP("BinocularsUI");
				UNK_0xE6C1DBD9(0);
				DESTROY_CAMERA(&uLocal_5);
				DESTROY_OBJECT(&uLocal_7);
				SET_ACTION_NODE_FOR_ACTOR(&iParam1, "item_binoculars/lower");
				DECOR_REMOVE(&Global_54076, "nquitbinos");
				return 0;
			}
			if (!GRINGO_IS_ACTOR_PLAYING_ANIM(&iParam1, &uVar0) && !DECOR_CHECK_EXIST(&Global_54076, "anthro04"))
			{
				return 0;
			}
			if (UNK_0xDA674AE0("@UI.CANCELMINIGAME", 1, 0))
			{
				if (!DECOR_CHECK_EXIST(&Global_54076, "anthro04"))
				{
			...
			if (IS_OBJECT_VALID(&uLocal_5) && UNK_0x00EF33EF())
			{
				fVar1 = GET_CURRENT_GAME_TIME();
				uVar2 = GET_CAMERA_FOV(&uLocal_5);
				UPDATE_AIMRAMP(&fVar3, &fVar4, &uLocal_7, "roamNew", 1);
				GET_OBJECT_ORIENTATION(&uLocal_5, &Var5);
				fVar7 = (TAN_DEGREE(uVar2) * 57,295f);
				Var5 = (Var5 + (fVar3 * fVar7));
				Var5.f_4 = (StackVal + (fVar4 * fVar7));
				Function_53(&Var5, 60.0f, -20.0f);
				SET_OBJECT_ORIENTATION(&uLocal_5, Var5);
				uVar8 = GET_OBJECT_FROM_ACTOR(&iParam1);
				GET_OBJECT_NAMED_BONE_POSITION(&uVar8, "head", &Var9);
				GET_OBJECT_POSITION(&uVar8, &Var11);
				uVar15 = Vector(0.0f, 0.0f, 0,1f);
				UNK_0xB89CC342(&iParam1, &uVar15, &Var17);
				Var9 = Vector(StackVal, StackVal, StackVal) + Vector(Vector(StackVal, StackVal, StackVal) - Vector(Var11, Var9, Var17), StackVal, StackVal);
				SET_CAMERA_POSITION(&uLocal_5, Var9);
			}
		}
		else if (!iLocal_4)
		{
			...
			{
				uVar19 = GET_CAMERA_FOV(&uLocal_5);
				...
				UPDATE_AIMRAMP(&fVar21, &fVar22, &uLocal_7, "roamNew", 1);
				GET_OBJECT_ORIENTATION(&uLocal_5, &Var23);
				fVar25 = (TAN_DEGREE(uVar19) * 57,295f);
				Var23 = (Var23 + (fVar21 * fVar25));
				Var23.f_4 = (StackVal + (fVar22 * fVar25));
				Function_53(&Var23, 89.0f, -89.0f);
				SET_OBJECT_ORIENTATION(&uLocal_5, Var23);
				uVar26 = GET_OBJECT_FROM_ACTOR(&iParam1);
				GET_OBJECT_NAMED_BONE_POSITION(&uVar26, "head", &Var27);
				GET_OBJECT_POSITION(&uVar26, &Var29);
				uVar33 = Vector(0.0f, 0.0f, 0,1f);
				UNK_0xB89CC342(&iParam1, &uVar33, &Var35);
				Var27 = Vector(StackVal, StackVal, StackVal) + Vector(Vector(StackVal, StackVal, StackVal) - Vector(Var29, Var27, Var35), StackVal, StackVal);
				SET_CAMERA_POSITION(&uLocal_5, Var27);
			}
		}
		else
		{
...
void Function_53(var uParam0, float fParam1, float fParam2) //Position: 0x3250 / 12880
{
	if (uParam0 > fParam2)
	{
		uParam0 = fParam2;
...
bool Function_54() //Position: 0x3279 / 12921
{
	var uVar0;
	struct<2> Var1;
	struct<2> Var3;
	...
	if (!IS_OBJECT_VALID(&uLocal_5))
	{
		uLocal_5 = CREATE_CAMERA_IN_LAYOUT(Function_55(), "bino_camera", 0);
	}
	if (!IS_OBJECT_VALID(&uLocal_7))
	{
		uLocal_7 = CREATE_AIMRAMP_IN_LAYOUT(Function_55(), "bino_camera_ar");
	}
	if (IS_OBJECT_VALID(&uLocal_5))
	{
		if (UNK_0x00EF33EF())
		{
			INIT_CAMERA_FROM_GAME_CAMERA(&uLocal_5);
			SET_CURRENT_CAMERA_ON_CHANNEL(&uLocal_5, 0, 0f, 0f, 0, 0, 0, 0, 0, 0);
			uVar0 = GET_OBJECT_FROM_ACTOR(&Global_54076);
			GET_OBJECT_NAMED_BONE_POSITION(&uVar0, "head", &Var1);
			GET_OBJECT_POSITION(&uVar0, &Var3);
			uVar7 = Vector(0.0f, 0.0f, -0,1f);
			UNK_0xB89CC342(&Global_54076, &uVar7, &Var9);
			Var1 = Vector(StackVal, StackVal, StackVal) + Vector(Vector(StackVal, StackVal, StackVal) - Vector(Var3, Var1, Var9), StackVal, StackVal);
			SET_CAMERA_POSITION(&uLocal_5, Var1);
			SET_CAMERA_NEAR_CLIP_PLANE(&uLocal_5, 0,35f);
			SET_CAMERA_FOV(&uLocal_5, 7,6f);
			SET_USING_BINOCULARS(1);
			if (!DECOR_CHECK_EXIST(&Global_54076, "anthro04"))
			{
				UI_PUSH("BinocularsUI");
			}
			UNK_0xE6C1DBD9(1);
			iLocal_9 = GET_GAME_CAMERA_AUTO_TILTING_STATE(2);
			iLocal_10 = GET_GAME_CAMERA_AUTO_CENTERING_STATE(2);
			iLocal_11 = GET_GAME_CAMERA_AUTO_TILTING_STATE(0);
			iLocal_12 = GET_GAME_CAMERA_AUTO_CENTERING_STATE(0);
			iLocal_13 = GET_GAME_CAMERA_AUTO_TILTING_STATE(1);
			iLocal_14 = GET_GAME_CAMERA_AUTO_CENTERING_STATE(1);
...
var Function_55() //Position: 0x33E9 / 13289
{
	int iVar0;
...
void Function_56(int iParam0, var uParam1, var uParam2) //Position: 0x345A / 13402
{
	var uVar0;
	var uVar1;
	int iVar2;
	...
	if (IS_GRINGO_COMPONENT_VALID(&uVar0) && uParam2)
	{
		if (IS_GRINGO_COMPONENT_VALID(&uVar1))
		{
			...
			{
				if (GRINGO_ATTACH_PROP_TO_ANIM(&uParam1, &uVar1, Function_58(&uVar1, "AttachBone", &iVar2), Function_58(&uVar1, "AttachLocator", &iVar2), 1))
				{
					Function_57(&uVar1, &uParam1);
					uParam2 = 1;
				}
			}
		}
...
void Function_57(var uParam0, int iParam1) //Position: 0x34F7 / 13559
{
	var uVar0;
	int iVar1;
	var uVar2;
...
var Function_58(var uParam0, var uParam1, int iParam2) //Position: 0x35BF / 13759
{
	int iVar0;
	int iVar1;
...
var Function_59(int iParam0) //Position: 0x360F / 13839
{
	return GRINGO_GET_FIRST_NAMED_CHILD("UseProp", 1, &iParam0);
}

var Function_60(int iParam0) //Position: 0x3626 / 13862
{
	return GRINGO_GET_FIRST_NAMED_CHILD("UseAnim", 2, &iParam0);
}

void Function_61(var uParam0, var uParam1, int iParam2) //Position: 0x363D / 13885
{
	Function_69("BINOCULARS: StartUse");
	uParam0 = Function_68(&uParam0);
	iParam2 = 0;
	if (DECOR_CHECK_EXIST(&uParam1, "BinocularsStart"))
	{
		DECOR_REMOVE(&uParam1, "BinocularsStart");
	}
	if ((IS_ACTOR_INSIDE_VEHICLE(&uParam1) && Global_54081 == 0) && Global_54081 == 1)
	{
		Function_66("item_binoculars_fail", 0x41200000, 1, 0, 2, 1, 0);
		iParam2 = 0;
		iLocal_4 = 0;
	}
	else
	{
		if (((Function_62(&uParam1, 0, 0) || IS_WEAPON_DRAWN(&uParam1)) || GET_WEAPON_IN_HAND(&uParam1) == 4294967295) || DECOR_CHECK_EXIST(&Global_54076, "anthro04"))
		{
			iLocal_4 = 0;
		}
		...
		else if (Function_44(Global_99130.f_40, 1))
		{
			iParam2 = 0;
			Function_66("item_binoculars_fail", 0x41200000, 1, 0, 2, 1, 0);
		}
		...
		else if (!IS_GRINGO_VALID(GET_CURRENT_GRINGO(&uParam1)) || !IS_ACTOR_VALID(GET_ATTACHED_HOGTIE_VICTIM(&uParam1)))
		{
		...
		if (IS_WEAPON_DRAWN(&uParam1) && iParam2)
		{
...
int Function_62(int iParam0, bool bParam1, bool bParam2) //Position: 0x3871 / 14449
{
...
bool Function_63() //Position: 0x3935 / 14645
{
	return ((Global_54082 != 0 || Global_54082 != 2) || Global_54082 != 3);
}

bool Function_64(int iParam0) //Position: 0x394C / 14668
{
	return IS_ACTOR_IN_HOGTIE(&iParam0);
}

bool Function_65(int iParam0) //Position: 0x3958 / 14680
{
	return IS_ACTOR_VALID(GET_LASSO_USER(&iParam0));
}

void Function_66(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6) //Position: 0x3967 / 14695
{
	struct<4> Var0;
	var uVar4;
	...
	if (Global_6623 != 1 && Global_10966 == 4294967295)
	{
		...
		{
			Var0 = { StackVal, StackVal, StackVal, Function_67(Global_10966) };
		}
...
struct<16> Function_67(int iParam0) //Position: 0x39F2 / 14834
{
	char* cVar0[16];
...
var Function_68(int iParam0) //Position: 0x3A31 / 14897
{
	return GRINGO_GET_FIRST_NAMED_CHILD("ItemAttribs", 4, &iParam0);
}

void Function_69(int iParam0) //Position: 0x3A4C / 14924
{
	LOG_OBJECT2(GRINGO_GET_MY_OBJECT_REF(), GET_TARGET_OBJECT(), &iParam0, 0, 0, 0);
	return;
}

bool Function_70(int iParam0, int iParam1) //Position: 0x3A61 / 14945
{
	var uVar0;
	var uVar1;
