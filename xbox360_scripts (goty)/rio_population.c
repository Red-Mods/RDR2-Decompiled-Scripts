//Decompiled with MagicRDR v1.0
//Function Count : 21
//Statics Count : 4
//Natives Count : 45
//Parameters Count : 2

#region Local Var
	struct<2> Local_0 = { 0, 0 } ;
	struct<2> ScriptParam_0 = { 0, 0 } ;
#endregion

void main() //Position: 0x0 / 0
{
	var uVar0;
	var uVar1;
	var uVar2;
	var uVar3;
	var uVar4;
	var uVar5;
	var uVar6;
	var uVar7;
	var uVar8;
	var uVar9;
	var uVar10;
	var uVar11;
	var uVar12;
	var uVar13;
	var uVar14;
	bool bVar15;
	bool bVar16;
	var uVar17;
	var uVar18;
	var uVar19;
	var uVar20;
	var uVar21;
	var uVar22;
	var uVar23;
	var uVar24;
	var uVar25;
	var uVar26;
	var uVar27;
	var uVar28;
	var uVar29;
	var uVar30;
	var uVar31;
	var uVar32;
	var uVar33;
	var uVar34;
	var uVar35;
	var uVar36;
	var uVar37;
	var uVar38;
	var uVar39;
	var uVar40;
	
	Local_0 = { StackVal, ScriptParam_0 };
	ENABLE_AMBIENT_SPAWNING(false);
	strcpy(&Global_13171, "Loa", 4);
	Function_20("Loading Rio Bravo population file", 3);
	Global_30839++;
	BEGIN_POPULATION_DEFINITION();
	Function_19(&(Global_30750[6]));
	Global_30750[6] = Function_18("animal_horse", 4);
	Function_17(&(Global_30750[6]), 976, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 977, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 983, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 984, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 985, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 986, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 987, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 988, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 989, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 990, 100.0f, 0, 3);
	Function_17(&(Global_30750[6]), 991, 100.0f, 0, 3);
	Function_19(&(Global_30750[15]));
	Global_30750[15] = Function_18("animal_horse_unsaddled", 0);
	Function_16(&(Global_30750[15]), 976, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 977, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 983, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 984, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 985, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 986, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 987, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 988, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 989, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 990, 100.0f, 0, 0);
	Function_16(&(Global_30750[15]), 991, 100.0f, 0, 0);
	Function_19(&(Global_30750[0]));
	Global_30750[0] = Function_18("ped_wilderness", Global_30840);
	Function_16(&(Global_30750[0]), 33, 2.0f, 0, 0);
	Function_16(&(Global_30750[0]), 34, 2.0f, 0, 0);
	Function_16(&(Global_30750[0]), 37, 2.0f, 0, 0);
	Function_16(&(Global_30750[0]), 38, 2.0f, 0, 0);
	Function_16(&(Global_30750[0]), 39, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 46, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 50, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 51, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 53, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 54, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 55, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 56, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 153, 8.0f, 0, 0);
	Function_16(&(Global_30750[0]), 173, 10.0f, 0, 0);
	Function_16(&(Global_30750[0]), 174, 10.0f, 0, 0);
	Function_16(&(Global_30750[0]), 175, 2.0f, 0, 0);
	Function_16(&(Global_30750[0]), 178, 10.0f, 0, 0);
	Function_16(&(Global_30750[0]), 199, 10.0f, 0, 0);
	Function_16(&(Global_30750[0]), 202, 10.0f, 0, 0);
	Function_19(&(Global_30750[1]));
	Global_30750[1] = Function_18("ped_traveller", 3);
	Function_16(&(Global_30750[1]), 64, 10.0f, 0, 0);
	Function_16(&(Global_30750[1]), 74, 10.0f, 0, 0);
	Function_16(&(Global_30750[1]), 48, 10.0f, 0, 0);
	Function_16(&(Global_30750[1]), 73, 10.0f, 0, 0);
	Function_16(&(Global_30750[1]), 78, 10.0f, 0, 0);
	Function_16(&(Global_30750[1]), 49, 10.0f, 0, 0);
	Function_19(&(Global_30750[5]));
	Global_30750[5] = Function_18("animal_wilderness", 1);
	Function_13(&(Global_30750[5]), 1057, 2.0f, 14, 1, 0);
	uVar0 = Function_18("critter_pop", 0);
	Function_13(&uVar0, 1057, 2.0f, 14, 1, 0);
	uVar1 = Function_18("armadillo_pop", 0);
	Function_16(&uVar1, 1081, 4.0f, 0, 0);
	Function_16(&uVar1, 1081, 4.0f, 0, 0);
	Function_16(&uVar1, 1081, 4.0f, 0, 0);
	Function_19(&(Global_30750[10]));
	Global_30750[10] = Function_18("animal_hunt", 1);
	Function_16(&(Global_30750[10]), 1084, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1088, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1089, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1090, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1050, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1050, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1051, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1052, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1052, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1119, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1119, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1120, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1116, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1117, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1116, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1061, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1062, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1063, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1054, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1055, 2.0f, 0, 0);
	Function_16(&(Global_30750[10]), 1056, 2.0f, 0, 0);
	Function_19(&(Global_30750[11]));
	Global_30750[11] = Function_18("animal_predator", 1);
	Function_16(&(Global_30750[11]), 1061, 3.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1062, 5.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1063, 5.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1088, 5.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1089, 5.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1090, 5.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1050, 3.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1050, 3.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1051, 3.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1052, 3.0f, 0, 0);
	Function_16(&(Global_30750[11]), 1052, 3.0f, 0, 0);
	Function_19(&(Global_30750[13]));
	Global_30750[13] = Function_18("animal_hunted", 0);
	Function_16(&(Global_30750[13]), 1061, 4.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1062, 4.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1063, 4.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1119, 2.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1119, 2.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1120, 2.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1116, 2.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1117, 2.0f, 0, 0);
	Function_16(&(Global_30750[13]), 1116, 2.0f, 0, 0);
	Function_19(&(Global_30750[12]));
	Global_30750[12] = Function_18("animal_prey", 0);
	Function_16(&(Global_30750[12]), 1118, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1119, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1119, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1120, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1116, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1117, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1116, 3.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1078, 7.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1078, 7.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1078, 7.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1026, 4.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1027, 4.0f, 0, 0);
	Function_16(&(Global_30750[12]), 1028, 4.0f, 0, 0);
	Function_19(&(Global_30750[8]));
	Global_30750[8] = Function_18("animal_herd", 1);
	Function_16(&(Global_30750[8]), 1013, 10.0f, 0, 0);
	Function_16(&(Global_30750[8]), 1014, 10.0f, 0, 0);
	Function_16(&(Global_30750[8]), 1008, 10.0f, 0, 0);
	Function_16(&(Global_30750[8]), 1009, 10.0f, 0, 0);
	Function_16(&(Global_30750[8]), 1010, 10.0f, 0, 0);
	Function_16(&(Global_30750[8]), 1011, 10.0f, 0, 0);
	uVar2 = Function_18("coyote_pop", 0);
	Function_16(&uVar2, 1050, 4.0f, 0, 0);
	Function_16(&uVar2, 1050, 4.0f, 0, 0);
	Function_16(&uVar2, 1051, 4.0f, 0, 0);
	Function_16(&uVar2, 1052, 4.0f, 0, 0);
	Function_16(&uVar2, 1052, 4.0f, 0, 0);
	uVar3 = Function_18("cougar_pop", 0);
	Function_16(&uVar3, 1089, 4.0f, 0, 0);
	Function_16(&uVar3, 1090, 4.0f, 0, 0);
	uVar4 = Function_18("animal_duck", 0);
	Function_16(&uVar4, 1140, 100.0f, 0, 0);
	Function_16(&uVar4, 1140, 100.0f, 0, 0);
	uVar5 = Function_18("deer_pop", 0);
	Function_16(&uVar5, 1119, 4.0f, 0, 0);
	Function_16(&uVar5, 1119, 4.0f, 0, 0);
	Function_16(&uVar5, 1120, 4.0f, 0, 0);
	Function_16(&uVar5, 1116, 1.0f, 0, 0);
	Function_16(&uVar5, 1117, 1.0f, 0, 0);
	Function_16(&uVar5, 1116, 1.0f, 0, 0);
	uVar6 = Function_18("wolf_pop", 0);
	Function_16(&uVar6, 1061, 4.0f, 0, 0);
	Function_16(&uVar6, 1062, 4.0f, 0, 0);
	Function_16(&uVar6, 1063, 4.0f, 0, 0);
	uVar7 = Function_18("fox_pop", 0);
	Function_16(&uVar7, 1054, 4.0f, 0, 0);
	Function_16(&uVar7, 1055, 4.0f, 0, 0);
	Function_16(&uVar7, 1056, 4.0f, 0, 0);
	uVar8 = Function_18("rabbit_pop", 0);
	Function_16(&uVar8, 1078, 4.0f, 0, 0);
	Function_16(&uVar8, 1078, 4.0f, 0, 0);
	Function_16(&uVar8, 1078, 4.0f, 0, 0);
	uVar9 = Function_18("nnight_pop", 0);
	Function_13(&uVar9, 1085, 4.0f, 49, 0, 0);
	Function_13(&uVar9, 1082, 4.0f, 33, 0, 0);
	Function_19(&(Global_30750[7]));
	Global_30750[7] = Function_18("animal_bird", 1);
	Function_13(&(Global_30750[7]), 1128, 6.0f, 15, 0, 0);
	Function_13(&(Global_30750[7]), 1131, 6.0f, 14, 0, 0);
	Function_13(&(Global_30750[7]), 1135, 2.0f, 14, 0, 0);
	Function_13(&(Global_30750[7]), 1136, 2.0f, 14, 0, 0);
	Function_13(&(Global_30750[7]), 1136, 2.0f, 14, 0, 0);
	Function_13(&(Global_30750[7]), 1145, 3.0f, 14, 0, 0);
	Function_13(&(Global_30750[7]), 1137, 3.0f, 33, 0, 0);
	uVar10 = Function_18("hawk", 0);
	Function_13(&uVar10, 1145, 3.0f, 15, 0, 0);
	uVar11 = Function_18("vulture", 0);
	Function_13(&uVar11, 1131, 6.0f, 14, 0, 0);
	uVar12 = Function_18("crow", 0);
	Function_13(&uVar12, 1131, 6.0f, 14, 0, 0);
	uVar13 = Function_18("nsmallbird", 0);
	Function_13(&uVar13, 1135, 2.0f, 15, 0, 0);
	Function_13(&uVar13, 1136, 2.0f, 15, 0, 0);
	Function_13(&uVar13, 1136, 2.0f, 15, 0, 0);
	Function_13(&uVar13, 1137, 2.0f, 33, 0, 0);
	Function_19(&(Global_30750[14]));
	Global_30750[14] = Function_18("birds_trees", 1);
	Function_16(&(Global_30750[14]), 1131, 100.0f, 0, 0);
	Function_16(&(Global_30750[14]), 1132, 100.0f, 0, 0);
	Function_16(&(Global_30750[14]), 1133, 100.0f, 0, 0);
	Function_16(&(Global_30750[14]), 1135, 100.0f, 0, 0);
	Function_16(&(Global_30750[14]), 1136, 100.0f, 0, 0);
	Function_16(&(Global_30750[14]), 1136, 100.0f, 0, 0);
	SAVE_POP_SET_BIRDS_SPAWNING_FROM_TREES(Global_30750[14]);
	uVar14 = Function_18("wildHorses", 0);
	Function_16(&uVar14, 980, 8.0f, 0, 0);
	Function_16(&uVar14, 981, 8.0f, 0, 0);
	Function_16(&uVar14, 984, 8.0f, 0, 0);
	Function_16(&uVar14, 987, 8.0f, 0, 0);
	Function_16(&uVar14, 989, 8.0f, 0, 0);
	Function_16(&uVar14, 990, 8.0f, 0, 0);
	if (Function_9(6, 0))
	{
		Function_16(&uVar14, 978, 10.0f, 0, 0);
	}
	Function_8(Global_30750[5], 9E-05f);
	Function_19(&(Global_30750[2]));
	Global_30750[2] = Function_18("ped_law", 1);
	Function_16(&(Global_30750[2]), 449, 1.0f, 0, 0);
	Function_16(&(Global_30750[2]), 436, 4.0f, 0, 0);
	Function_16(&(Global_30750[2]), 437, 8.0f, 0, 0);
	Function_16(&(Global_30750[2]), 438, 8.0f, 0, 0);
	Function_16(&(Global_30750[2]), 440, 8.0f, 0, 0);
	Function_19(&(Global_30750[3]));
	Global_30750[3] = Function_18("ped_bad_guys_local", 1);
	Function_16(&(Global_30750[3]), 486, 10.0f, 0, 0);
	Function_16(&(Global_30750[3]), 487, 10.0f, 0, 0);
	Function_16(&(Global_30750[3]), 488, 10.0f, 0, 0);
	Function_16(&(Global_30750[3]), 489, 3.0f, 0, 0);
	Function_16(&(Global_30750[3]), 490, 3.0f, 0, 0);
	Function_16(&(Global_30750[3]), 491, 3.0f, 0, 0);
	Function_16(&(Global_30750[3]), 492, 1.0f, 0, 0);
	Function_16(&(Global_30750[3]), 493, 1.0f, 0, 0);
	Function_19(&(Global_30750[4]));
	Global_30750[4] = Function_18("ped_bad_guys_generic", 1);
	Function_16(&(Global_30750[4]), 403, 1.0f, 0, 0);
	Function_16(&(Global_30750[4]), 404, 1.0f, 0, 0);
	Function_16(&(Global_30750[4]), 405, 1.0f, 0, 0);
	Function_16(&(Global_30750[4]), 400, 4.0f, 0, 0);
	Function_16(&(Global_30750[4]), 401, 4.0f, 0, 0);
	Function_16(&(Global_30750[4]), 402, 4.0f, 0, 0);
	Function_16(&(Global_30750[4]), 397, 8.0f, 0, 0);
	Function_16(&(Global_30750[4]), 397, 8.0f, 0, 0);
	Function_16(&(Global_30750[4]), 398, 8.0f, 0, 0);
	Function_19(&(Global_30750[9]));
	Global_30750[9] = Function_18("ped_vehicle", 1);
	Function_16(&(Global_30750[9]), 1183, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1188, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1200, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1199, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1195, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1196, 2.0f, 0, 0);
	Function_16(&(Global_30750[9]), 1201, 2.0f, 0, 0);
	while (!Function_7(&(Global_29008[Local_0]), 16) && !IS_EXITFLAG_SET())
	{
		WAIT(false);
	}
	if (IS_EXITFLAG_SET())
	{
		Function_6("Exiting population file early", 5.0f);
		END_POPULATION_DEFINITION();
		Global_30839 = (Global_30839 - 1);
		if (Global_30839 == 0)
		{
			ENABLE_AMBIENT_SPAWNING(Function_5(1));
		}
		TERMINATE_THIS_SCRIPT();
	}
	bVar15 = FIND_NAMED_LAYOUT("RioBravo_layout");
	if (IS_LAYOUTREF_VALID(bVar15))
	{
		bVar16 = FIND_VOLUME_IN_LAYOUT(bVar15, "riov_flk_CENTER_set");
		if (IS_VOLUME_VALID(bVar16))
		{
			if (!Function_4())
			{
				uVar17 = Function_3(bVar16, uVar14, 1.5E-05f, 0, 1, 1);
				SET_ZONE_POPULATION_IS_FLOCK(uVar17, 1);
				SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar17, 1);
				SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar17, 3);
			}
			uVar18 = Function_3(bVar16, uVar2, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar18, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar18, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar18, 3);
			uVar19 = Function_3(bVar16, uVar8, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar19, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar19, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar19, 2);
			uVar20 = Function_3(bVar16, uVar5, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar20, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar20, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar20, 3);
			uVar21 = Function_3(bVar16, uVar3, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar21, 1);
			uVar22 = Function_3(bVar16, uVar0, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar22, 2);
			uVar23 = Function_3(bVar16, uVar9, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar23, 2);
		}
		else
		{
			LOG_ERROR("cho_pop: Cannot find riov_flk_CENTER_set to create population volumes");
		}
		bVar16 = FIND_VOLUME_IN_LAYOUT(bVar15, "riov_flk_SOUTH_set");
		if (IS_VOLUME_VALID(bVar16))
		{
			if (!Function_4())
			{
				uVar24 = Function_3(bVar16, uVar14, 1.5E-05f, 0, 1, 1);
				SET_ZONE_POPULATION_IS_FLOCK(uVar24, 1);
				SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar24, 1);
				SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar24, 3);
			}
			uVar25 = Function_3(bVar16, uVar2, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar25, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar25, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar25, 3);
			uVar26 = Function_3(bVar16, uVar7, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar26, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar26, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar26, 2);
			uVar27 = Function_3(bVar16, uVar6, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar27, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar27, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar27, 3);
			uVar28 = Function_3(bVar16, uVar5, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar28, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar28, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar28, 3);
			uVar29 = Function_3(bVar16, uVar3, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar29, 1);
			uVar30 = Function_3(bVar16, uVar0, 9E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar30, 2);
			uVar31 = Function_3(bVar16, uVar9, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar31, 2);
			uVar32 = Function_3(bVar16, uVar8, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar32, 2);
		}
		else
		{
			LOG_ERROR("cho_pop: Cannot find riov_flk_SW_set to create population volumes");
		}
		bVar16 = FIND_VOLUME_IN_LAYOUT(bVar15, "riov_flk_NORTH_set");
		if (IS_VOLUME_VALID(bVar16))
		{
			if (!Function_4())
			{
				uVar33 = Function_3(bVar16, uVar14, 1.5E-05f, 0, 1, 1);
				SET_ZONE_POPULATION_IS_FLOCK(uVar33, 1);
				SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar33, 1);
				SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar33, 3);
			}
			uVar34 = Function_3(bVar16, uVar2, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar34, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar34, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar34, 3);
			uVar35 = Function_3(bVar16, uVar6, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar35, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar35, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar35, 3);
			uVar36 = Function_3(bVar16, uVar8, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar36, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar36, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar36, 2);
			uVar37 = Function_3(bVar16, uVar5, 2.7E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_IS_FLOCK(uVar37, 1);
			SET_ZONE_POPULATION_MIN_FLOCK_SIZE(uVar37, 1);
			SET_ZONE_POPULATION_MAX_FLOCK_SIZE(uVar37, 3);
			uVar38 = Function_3(bVar16, uVar3, 2.1E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar38, 1);
			uVar39 = Function_3(bVar16, uVar0, 9E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar39, 2);
			uVar40 = Function_3(bVar16, uVar9, 6E-05f, 0, 0, 1);
			SET_ZONE_POPULATION_MAX_PER_CELL(uVar40, 2);
		}
		else
		{
			LOG_ERROR("cho_pop: Cannot find riov_flk_NORTH_set to create population volumes");
		}
		bVar16 = FIND_VOLUME_IN_LAYOUT(bVar15, "riov_aquaticWildlife_set");
		if (IS_VOLUME_VALID(bVar16))
		{
			Function_3(bVar16, uVar9, 6E-05f, 0, 0, 1);
			Function_3(bVar16, uVar4, 4.5E-05f, 0, 0, 1);
		}
		else
		{
			LOG_ERROR("hen_pop: Cannot find riov_aquaticWildlife_set to create population volumes");
		}
		bVar16 = FIND_VOLUME_IN_LAYOUT(bVar15, "riov_flk_BIRD_set");
		if (IS_VOLUME_VALID(bVar16))
		{
			Function_3(bVar16, uVar10, 6E-05f, 0, 0, 1);
			Function_3(bVar16, uVar11, 6E-05f, 0, 0, 1);
			Function_3(bVar16, uVar12, 6E-05f, 0, 0, 1);
			Function_3(bVar16, uVar13, 9E-05f, 0, 0, 1);
		}
		else
		{
			LOG_ERROR("cho_pop: Cannot find riov_flk_BIRD_set to create population volumes");
		}
	}
	else
	{
		LOG_ERROR("cho_pop: Cannot find one of the volumes to create population volumes");
	}
	END_POPULATION_DEFINITION();
	Global_30839 = (Global_30839 - 1);
	SET_AMBIENT_FORCE_WAIT_STATE(0);
	while (!Function_2() && !IS_EXITFLAG_SET())
	{
		WAIT(false);
	}
	if (!Function_7(&(Global_29008[Local_0]), 128) && Global_30839 != 0)
	{
		ENABLE_AMBIENT_SPAWNING(Function_5(1));
	}
	Global_13171 = Global_29155[Local_010].f_20;
	Function_6("Finished loading Rio Bravo population file", 5.0f);
	Function_1(&(Global_29008[Local_0]), 64);
	TERMINATE_THIS_SCRIPT();
	return;
}

void Function_1(var uParam0, var uParam1) //Position: 0x1791 / 6033
{
	*uParam0 = (*uParam0 || uParam1);
	return;
}

bool Function_2() //Position: 0x17A0 / 6048
{
	int iVar0;
	
	if (!Function_5(1))
	{
		return 1;
	}
	iVar0 = 0;
	while (iVar0 <= Global_30750)
	{
		if (!IS_POPSET_VALID(Global_30750[iVar0]))
		{
		}
		else if (IS_POPULATION_SET_REQUIRED_RESIDENT(Global_30750[iVar0]))
		{
			if (!IS_POPULATION_SET_READY(Global_30750[iVar0], false, 4294967295))
			{
				return 0;
			}
		}
		iVar0++;
	}
	return 1;
}

var Function_3(bool bParam0, var uParam1, float fParam2, int iParam3, bool bParam4, bool bParam5) //Position: 0x17F5 / 6133
{
	bool bVar0;
	
	bVar0 = CREATE_ZONE_VOLUME(bParam0);
	SET_ZONE_POPULATION_TYPE(bVar0, uParam1);
	SET_ZONE_POPULATION_DENSITY(bVar0, fParam2);
	SET_ZONE_RESTRICT_ACTORS(bVar0, iParam3);
	if (bParam4)
	{
		SET_ZONE_POPULATION_ANIMAL_DOMESTICATION(bVar0, 1);
	}
	SET_ZONE_ALLOWED_SPAWN_READY_TOO_CLOSE(bVar0, !bParam5);
	if (DECOR_CHECK_EXIST(bParam0, "locflag"))
	{
		DECOR_SET_INT(bParam0, "locflag", (1 || DECOR_GET_INT(bParam0, "locflag")));
	}
	else
	{
		DECOR_SET_INT(bParam0, "locflag", true);
	}
	return bVar0;
}

bool Function_4() //Position: 0x1875 / 6261
{
	return NET_IS_MANAGER_INITIALIZED();
}

bool Function_5(int iParam0) //Position: 0x187E / 6270
{
	int iVar0;
	
	iVar0 = (Global_26119 && iParam0);
	if (!iVar0 != 0)
	{
		return 1;
	}
	return 0;
}

void Function_6(bool bParam0, float fParam1) //Position: 0x189A / 6298
{
	if (!Function_5(128))
	{
		return;
	}
	if (!STRING_CONTAINS_STRING(&Global_13133, bParam0))
	{
		strcpy(&Global_13133, bParam0, 64);
		Global_13133.f_68 = 2;
		Global_13133.f_64 = GET_CURRENT_GAME_TIME();
		Global_13133.f_72 = (Global_13133.f_64 + fParam1);
	}
	return;
}

bool Function_7(var uParam0, int iParam1) //Position: 0x18D8 / 6360
{
	int iVar0;
	
	iVar0 = (*uParam0 && iParam1);
	if (!iVar0 != 0)
	{
		return 1;
	}
	return 0;
}

void Function_8(var uParam0, bool bParam1) //Position: 0x18F4 / 6388
{
	SET_DEFAULT_POPULATION_TYPE(uParam0);
	SET_DEFAULT_POPULATION_DENSITY(bParam1);
	return;
}

bool Function_9(var uParam0, bool bParam1) //Position: 0x1904 / 6404
{
	int iVar0;
	
	iVar0 = Function_11(uParam0);
	if (!Function_10(iVar0))
	{
		return 0;
	}
	if (StackVal == 3)
	{
		return 1;
	}
	if (!bParam1)
	{
		if (StackVal == 4)
		{
			return 1;
		}
	}
	return 0;
}

bool Function_10(int iParam0) //Position: 0x1941 / 6465
{
	if (iParam0 >= 0 || iParam0 <= 275)
	{
		return 0;
	}
	return 1;
}

var Function_11(int iParam0) //Position: 0x1958 / 6488
{
	if (!Function_12(iParam0))
	{
		return 4294967295;
	}
	return StackVal;
}

bool Function_12(bool bParam0) //Position: 0x1972 / 6514
{
	if (bParam0 >= 0 || bParam0 <= 58)
	{
		return 0;
	}
	return 1;
}

var Function_13(var uParam0, bool bParam1, float fParam2, int iParam3, bool bParam4, bool bParam5) //Position: 0x1988 / 6536
{
	int iVar0;
	int iVar1;
	
	iVar0 = GET_ACTOR_ENUM_FACTION(bParam1);
	iVar1 = GET_ACTORENUM_SPECIES(bParam1);
	if (iVar0 <= 32)
	{
		if (!Global_30804[iVar0] && !bParam5)
		{
			return 4294967295;
		}
	}
	if (iVar1 <= 36)
	{
		if (!Global_30767[iVar1])
		{
			return 4294967295;
		}
	}
	if (!IS_POPSET_VALID(*uParam0))
	{
		*uParam0 = CREATE_POPULATION_SET(0);
	}
	LINK_ACTORENUM_TO_POPULATION_TIMED(*uParam0, bParam1, fParam2, Function_15(iParam3), Function_14(iParam3));
	if (bParam4)
	{
	}
	return (GET_NUM_ACTORENUMS_IN_POPULATION(*uParam0) - 1);
}

var Function_14(int iParam0) //Position: 0x1A00 / 6656
{
	int iVar0;
	
	iVar0 = 0;
	if (iParam0 & 33 == 32)
	{
		iVar0 = 5;
	}
	else if (iParam0 & 3 == 1)
	{
		iVar0 = 7;
	}
	else if (iParam0 & 6 == 2)
	{
		iVar0 = 11;
	}
	else if (iParam0 & 12 == 4)
	{
		iVar0 = 14;
	}
	else if (iParam0 & 24 == 8)
	{
		iVar0 = 18;
	}
	else if (iParam0 & 48 == 16)
	{
		iVar0 = 22;
	}
	return MAKE_TIME_OF_DAY(iVar0, 0, 0);
}

var Function_15(int iParam0) //Position: 0x1A6C / 6764
{
	int iVar0;
	
	iVar0 = 0;
	if (iParam0 & 33 == 1)
	{
		iVar0 = 5;
	}
	else if (iParam0 & 3 == 2)
	{
		iVar0 = 7;
	}
	else if (iParam0 & 6 == 4)
	{
		iVar0 = 11;
	}
	else if (iParam0 & 12 == 8)
	{
		iVar0 = 14;
	}
	else if (iParam0 & 24 == 16)
	{
		iVar0 = 18;
	}
	else if (iParam0 & 48 == 32)
	{
		iVar0 = 22;
	}
	return MAKE_TIME_OF_DAY(iVar0, 0, 0);
}

var Function_16(var uParam0, bool bParam1, float fParam2, bool bParam3, bool bParam4) //Position: 0x1AD8 / 6872
{
	int iVar0;
	int iVar1;
	
	iVar0 = GET_ACTOR_ENUM_FACTION(bParam1);
	iVar1 = GET_ACTORENUM_SPECIES(bParam1);
	if (IS_FACTION_VALID(iVar0))
	{
		if (iVar0 <= 32)
		{
			if (!Global_30804[iVar0] && !bParam4)
			{
				return 4294967295;
			}
		}
	}
	if (iVar1 < 4294967295 && iVar1 > 36)
	{
		if (!Global_30767[iVar1])
		{
			return 4294967295;
		}
	}
	if (!IS_POPSET_VALID(*uParam0))
	{
		*uParam0 = CREATE_POPULATION_SET(0);
	}
	LINK_ACTORENUM_TO_POPULATION(*uParam0, bParam1, fParam2);
	if (bParam3)
	{
	}
	return (GET_NUM_ACTORENUMS_IN_POPULATION(*uParam0) - 1);
}

var Function_17(var uParam0, bool bParam1, float fParam2, int iParam3, int iParam4) //Position: 0x1B54 / 6996
{
	int iVar0;
	int iVar1;
	
	iVar0 = Function_16(uParam0, bParam1, fParam2, iParam3, 0);
	if (iVar0 > 0)
	{
		iVar1 = GET_ACTORENUM_SPECIES(bParam1);
		if ((iVar1 != 23 || iVar1 != 24) || iVar1 != 0)
		{
			SET_ACCESSORYSET_ON_SPAWN(*uParam0, iVar0, iParam4);
		}
	}
	return iVar0;
}

var Function_18(bool bParam0, int iParam1) //Position: 0x1B99 / 7065
{
	return CREATE_NAMED_POPULATION_SET(bParam0, iParam1);
}

void Function_19(int iParam0) //Position: 0x1BA6 / 7078
{
	DESTROY_POPULATION_SET(*iParam0);
	return;
}

void Function_20(bool bParam0, var uParam1) //Position: 0x1BB2 / 7090
{
	if (!Function_5(128))
	{
		return;
	}
	if (!STRING_CONTAINS_STRING(&Global_13133, cParam0))
	{
		strcpy(&Global_13133, bParam0, 64);
		Global_13133.f_68 = 1;
		Global_13133.f_64 = GET_CURRENT_GAME_TIME();
		Global_13133.f_72 = Global_13133.f_64;
	}
	return;
}

